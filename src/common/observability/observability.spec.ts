...
describe('Observability contracts', () => {
  ...
  it('isolates concurrent async correlation contexts', async () => {
    const ids = await Promise.all(
      ['a', 'b'].map((requestId) =>
        correlation.run({ requestId }, async () => {
  ...
  it('never serializes nested secret-bearing errors or metadata', () => {
    const error = new Error('postgres://user:password@host/db token=secret', {
      cause: new Error('email@example.com password=hidden'),
    });
    const result = JSON.stringify(
      sanitize({ password: 'hidden', nested: { authorization: 'Bearer secret', err: error } }),
    );
    for (const secret of [
      'password@host',
      'token=secret',
  ...
  it.each([
    ...
    ['42P10', 500, 'DATABASE_SCHEMA_MISMATCH'],
    ['P2002', 409, 'RESOURCE_CONFLICT'],
  ])('maps database %s safely', (code, status, expected) => {
  ...
  it('returns real HTTP failures with correlation and no internal cause', () => {
    ...
    const filter = new AllExceptionsFilter(
      ...
      { setContext: jest.fn(), error, warn } as unknown as PinoLogger,
    );
    const id = requestId(null);
    const host = {
      switchToHttp: () => ({
        getRequest: () => ({ id, url: '/api/v1/generate-affiliate?token=hidden' }),
        getResponse: () => ({}),
      }),
    ...
    expect(reply.mock.calls[0]![1]).toMatchObject({
      ...
      requestId: id,
      path: '/api/v1/generate-affiliate',
    });
    ...
    filter.catch(
      new BadRequestException({
        ...
        message: 'Invalid',
        details: [{ field: 'url', code: 'isUrl', message: 'Invalid' }],
      }),
      host,
    );
