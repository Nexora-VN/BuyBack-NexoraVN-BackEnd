...
describe('Shopee URL boundary', () => {
  ...
  it('accepts canonical and slug URLs without fetching', async () => {
    ...
    expect(await makeCleanShortLink('https://shopee.vn/product/123/456?x=y')).toBe(
      'https://shopee.vn/product/123/456',
    );
    expect(parseShopeeProductUrl('https://shopee.vn/Phone-i.123.456')).toEqual({
      shopId: 123,
      productId: 456,
    });
  ...
  it('rejects arbitrary hosts before network access and checks redirects', async () => {
    ...
    expect(fetcher).not.toHaveBeenCalled();
    await expect(makeCleanShortLink('https://vn.shp.ee/test')).rejects.toThrow(
      'INVALID_SHOPEE_URL',
    );
