  ...
  async report(cookie: string, startDate: string, endDate: string, page: number) {
    for (let attempt = 0; attempt <= 3; attempt++) {
      try {
        ...
        if (response.status === 403) { await response.body?.cancel(); throw new ProviderError('PROVIDER_AUTH_EXPIRED'); }
        if (response.status === 400) { await response.body?.cancel(); throw new ProviderError('PROVIDER_BAD_REQUEST'); }
        if (response.status === 429 || response.status >= 500) { await response.body?.cancel(); throw new ProviderError('PROVIDER_RETRYABLE'); }
        if (!response.ok) { await response.body?.cancel(); throw new ProviderError('PROVIDER_HTTP_ERROR'); }
        const text = await response.text();
        if (text.length > 10000000) throw new ProviderError('PROVIDER_RESPONSE_TOO_LARGE');
        let parsed: unknown;
        try { parsed = decodeProvider(text); } catch { throw new ProviderError('PROVIDER_INVALID_JSON'); }
