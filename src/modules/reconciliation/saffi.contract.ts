...
export const itemSchema = z
  .object({
    ...
    item_name: z.string(),
    display_item_status: z.string(),
    affiliate_item_status: count,
    is_fraud: count,
    fraud_status: count,
    item_price: raw,
    actual_amount: raw,
    refunded_amount: raw,
    item_commission: raw,
    capped_brand_commission: raw,
    brand_commission_rate: count,
    platform_commission_rate: count,
  })
  .passthrough();
...
export const checkoutSchema = z
  .object({
    ...
    affiliate_net_commission: raw,
    estimated_total_commission: raw,
    gross_commission: raw,
    capped_commission: raw,
    total_brand_commission: raw,
...
export function decodeProvider(text: string): unknown {
  return parse(text, (_key: string, value: unknown) =>
    isLosslessNumber(value) ? value.value : value,
  );
}
...
export function conversionState(checkout: SaffiCheckout) {
  const orderStates = checkout.orders.map((order) => {
    ...
    if (!order.items.every((i) => [1, 2, 3, 4].includes(i.affiliate_item_status)))
      return 'MANUAL_REVIEW';
    const rejected = order.items.filter(
      (i) => i.affiliate_item_status === 3 || i.is_fraud === 1,
    ).length;
    if (rejected === order.items.length) return 'REJECTED';
    if (rejected) return 'PARTIALLY_VALIDATED';
    if (order.display_order_status === 2 && order.items.every((i) => i.affiliate_item_status === 2))
      return 'VALIDATED';
  ...
  if (new Set(orderStates).size > 1 || orderStates.includes('PARTIALLY_VALIDATED'))
