...
describe('Saffi report contract', () => {
  it('parses real Pending, Cancelled, Completed and fraud records without losing integers', () => {
    const data = envelopeSchema.parse(
      decodeProvider(readFileSync('test/fixtures/saffi-conversions.json', 'utf8')),
    );
    const records = data.data.list.map((row) => checkoutSchema.parse(row));
    expect(records.map(conversionState)).toEqual([
      'PENDING',
      'REJECTED',
      'REJECTED',
      'VALIDATED',
  ...
  it('validates real calendar dates and the 90-day maximum', () => {
    expect(syncInput.safeParse({ startDate: '2026-02-30', endDate: '2026-03-01' }).success).toBe(
      false,
    );
    expect(syncInput.safeParse({ startDate: '2026-01-01', endDate: '2026-06-01' }).success).toBe(
      false,
    );
  });
});
describe('Saffi client failure handling', () => {
  ...
  it('does not retry expired cookies or leak request secrets', async () => {
    ...
    await expect(
      new SaffiClient().report('secret-cookie', '2026-09-01', '2026-09-01', 1),
    ...
    jest.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
          code: 1,
          msg: 'bad',
          data: { page_num: 1, page_size: 100, total_count: 0, list: [] },
        }),
      ),
    );
    await expect(new SaffiClient().report('cookie', '2026-09-01', '2026-09-01', 1)).rejects.toThrow(
  ...
  it('retries a transient HTTP error', async () => {
    ...
    expect(
      (await new SaffiClient().report('cookie', '2026-09-01', '2026-09-01', 1)).report.total_count,
    ).toBe(0);
  });
});
