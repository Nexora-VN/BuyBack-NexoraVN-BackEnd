...
describe('batch settlement read model', () => {
  it('matches per-row blockers with three queries independent of page size', async () => {
    ...
    const db = {
      ...
      reconciliationIssue: {
        findMany: jest.fn().mockResolvedValue([{ provider: 'ADDLIVETAG:a', checkoutId: '1' }]),
        count: jest.fn().mockResolvedValue(1),
      },
      commission: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ checkout: { provider: 'ADDLIVETAG:b', checkoutId: '1' } }]),
        findFirst: jest.fn().mockResolvedValue({ id: 'duplicate' }),
      },
    };
    const checkout = {
      provider: 'ADDLIVETAG:a',
      accountId: 'account',
      checkoutId: '1',
      payload: {},
    };
    ...
    jest.clearAllMocks();
    const batch = await batchProviderSettlementBlockers(
      db as unknown as Tx,
